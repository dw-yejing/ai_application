...
def data_preprocess():
    ...
    hf_train_data_transforms = transforms.Compose([
        transforms.RandomResizedCrop(224),  # 224
        transforms.RandomHorizontalFlip(1),  # 默认概率0.5
        transforms.ToTensor(),
        # 图像标准化处理
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    vf_train_data_transforms = transforms.Compose([
        transforms.RandomResizedCrop(224),  # 224
        transforms.RandomVerticalFlip(1),  # 默认概率0.5
        transforms.ToTensor(),
        # 图像标准化处理
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    r_train_data_transforms = transforms.Compose([
        transforms.RandomResizedCrop(224),  # 224
    ...
    hf_train_data = ImageFolder(train_data_dir, transform=hf_train_data_transforms)
    vf_train_data = ImageFolder(train_data_dir, transform=vf_train_data_transforms)
    r_train_data = ImageFolder(train_data_dir, transform=r_train_data_transforms)
    train_data = train_data + hf_train_data + vf_train_data + r_train_data
